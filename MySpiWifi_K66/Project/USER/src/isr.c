...
void PIT1_IRQHandler(void)//5ms
{
  ...
  {
    count1 = 0;
    int32 num1 = ftm_quad_get(ftm1);//获取脉冲计数，带正负
    ftm_quad_clean(ftm1);//计数清零
    int32 num2 = ftm_quad_get(ftm2);//获取脉冲计数，带正负
    ftm_quad_clean(ftm2);//计数清零
    SensorEvent event1 = {SENSOR_SPEED, LEFT_ENC, num1, 0, 0, 0, 0};
    SensorEvent event2 = {SENSOR_SPEED, RIGHT_ENC, num2, 0, 0, 0, 0};
    onSpeedSensorUpdated(event1);
    onSpeedSensorUpdated(event2);
  }
  PIT_FlAG_CLR(pit1);
}

void PIT2_IRQHandler(void)//1ms
{
  onTimer1Updated();//定时器1
  onTimer2Updated();//定时器2
  onTimer3Updated();//定时器3
  PIT_FlAG_CLR(pit2);
}
