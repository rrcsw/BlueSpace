...
void M8266WIFI_Config(void)
{
  u16 status = 0;

  if(M8266WIFI_SPI_Setup_Connection(1, 0, "192.168.4.2", 8266, 4, 20, &status) == 0)
  {
    while(1)
    {
      printf("TCP通信连接失败！-WIFI_Func.c L.48\r\n");
...
//#define TEST_REMOTE_PORT       8266
//  
////#define TEST_REMOTE_ADDR       "10.22.123.203"   // "www.baidu.com"
////#define TEST_REMOTE_PORT       6000// 80
//#endif			 
//  
//  
...
//    
//    snd_data[0] = '1';
//    for(i=1; i<TEST_SEND_DATA_SIZE; i++) snd_data[i]='A';
//    
//    link_no=0;
//    
//    for(sent=0;sent<TEST_SEND_DATA_SIZE;sent++)
//    {
//      total_sent = M8266WIFI_SPI_Send_Data(snd_data+sent, TEST_SEND_DATA_SIZE-sent, link_no, &status);
//      printf("%lx\r\n",&status);
//    }
////    for(batch=0; ; batch++)
////    {
